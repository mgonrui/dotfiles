...
int main(void){
    ...
    for (k = 0; k < 5; k++){
        ...
        low_quiz_score[k] = 0;
        for (j = 0; j < 5; j++){
            if (high_quiz_score[k] < grid[j][k]){
                high_quiz_score[k] = grid[j][k];
            }
            if (low_quiz_score[k] > grid[j][k]){
                low_quiz_score[k] = grid[j][k];
            }
        }
    }
    for (j = 0; j < 5; j++){
        printf(" highest score of quiz n%d %d \n", j, high_quiz_score[j]);
        printf(" lowest score of quiz n%d %d \n", j, low_quiz_score[j]);
