...
DIR="${0%/*}"
"$DIR"/"$(find "$DIR" -type f -exec basename {} \; | grep '^_' | sort | kitty bash -c "fzf $* < /proc/$$/fd/0 > /proc/$$/fd/1" #fzf -e -i --delimiter _ --with-nth='2..' --reverse --prompt="fzf: ")" 

# selected="$(cat ~/.scripts/fzf/options.txt | fzf -e -i  --reverse --prompt="fzf: ")"

# if [ "$selected" == "pdf" ]; then
#     sh  ~/.scripts/fzf/_locate 
# fi
