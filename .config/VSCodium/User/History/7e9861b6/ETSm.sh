...
DIR="${0%/*}"
selected_script="$("$DIR"/"$(find "$DIR" -type f -exec basename {} \; | grep '^_' | sort | fzf -e -i --delimiter _ --with-nth='2..' --reverse --prompt="fzf: ")")"

if [ -n "$selected_script" ]; then
    # Launch the selected script with a delay of 1 second
    nohup "$DIR/$selected_script" >/dev/null 2>&1 &
    sleep 1
fi
